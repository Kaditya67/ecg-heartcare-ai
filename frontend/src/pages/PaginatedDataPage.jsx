...
const PaginatedDataPage = () => {
  ...
  // Label changes persisted locally
  ...
  // Fetch all files on mount and select all by default
  useEffect(() => {
    API.get('/files/summary/')
      .then(resp => {
        ...
        const initialFiles = filesData.map((f, i) => ({
          ...f,
          selected: i === 0,  // only first file selected initially
        }));
  ...
  // Fetch ECG records whenever page or selected files change
  useEffect(() => {
    ...
    fetchData(page, selectedFileNames);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [page, files]);

  // Main data fetch function with file filtering
  const fetchData = async (pageNum = 1, selectedFileNames = null) => {
    setLoading(true);
    setError('');
    try {
      const filesToUse = selectedFileNames ?? files.filter(f => f.selected).map(f => f.file_name);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
      ...
      <main style={{ flexGrow: 1, padding: '1.5rem', margin: 'auto', width: '100%' }}>
        <div className="max-w-6xl mx-auto p-6 space-y-6" style={{ backgroundColor: 'var(--bg)', color: 'var(--text)', minHeight: '100vh' }}>
              ...
              <div className="flex flex-col md:flex-row gap-8 items-start">
                <div className="flex-1 flex flex-col items-center">
                  ...
                  <div className="w-full" style={{ minHeight: '400px', maxWidth: 900 }}>
                    <ECGCharts ecgArray={ecgArray} chartType={chartType} theme={readyTheme} key={readyTheme + chartType} />
                  </div>
                </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {labelOptions.map((opt) => {
                        ...
                        return (
                          <button
                            key={opt.value}
                            className={`px-3 py-1 rounded text-xs font-medium ${isSelected ? 'text-white' : 'text-[var(--text)] border'}`}
          ...
          {/* Changes control buttons */}
          <div className="flex space-x-2 mb-6">
            <button
              disabled={!hasUnsavedChanges}
              onClick={handleSaveLabels}
              className={`px-4 py-2 rounded text-white ${hasUnsavedChanges ? 'bg-[var(--accent)] hover:bg-blue-700' : 'bg-gray-400 cursor-not-allowed'}`}
            >
            ...
            <button
              ...
              className={`px-4 py-2 rounded text-white ${hasUnsavedChanges ? 'bg-red-500 hover:bg-red-600' : 'bg-gray-400 cursor-not-allowed'}`}
            >
              Clear Changes
            </button>
          </div>
          ...
          {!loading && !error && (
            <>
              <div className="overflow-x-auto border border-[var(--border)] rounded bg-[var(--card-bg)] shadow" style={{ maxHeight: '300px', overflowY: 'auto' }}>
                <table className="min-w-full table-auto text-sm divide-y divide-gray-200">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {data.map(({ id, patient_id, heart_rate, label }, idx) => {
                      ...
                      return (
                        <tr key={id} className={`cursor-pointer hover:bg-[var(--highlight)] ${idx % 2 === 0 ? 'bg-[var(--card-bg)]' : ''}`}>
                          ...
                          <td className={`px-4 py-3 text-center font-medium ${isChanged ? 'text-green-600' : 'text-[var(--text)]'}`}>
                            {displayLabel}
                          </td>
                          <td className="px-4 py-3 text-center">
                            <button
                              ...
                              className={`px-3 py-1 rounded text-white transition ${loadingPlotId === id ? 'bg-gray-400 cursor-not-allowed' : 'bg-[var(--accent)] hover:bg-blue-700'}`}
              ...
              <div className="flex justify-center items-center gap-6 mt-6 select-none">
                ...
                <div className="flex items-center space-x-2">
                  <label htmlFor="pageInput" className="font-semibold text-[var(--text)]">Page</label>
                  <input
                    id="pageInput"
                    type="number"
                    min="1"
